...
def create_tour(nodes):
    ...
    for i in range(1, nodes_count):
        next_edge = (nodes[i - 1], nodes[i])
        result_tour.append(next_edge)

    last_edge = (nodes[nodes_count - 1], nodes[0])
    result_tour.append(last_edge)
    return result_tour
...
def test():
    ...
    is_correct = is_eulerian_tour(nodes, tour)
    print(is_correct)
